...
<!-- figure-spec: 意図=どのサイズが借りられやすいかという傾向を一目で読める形にし、相対度数にもとづく意思決定の土台にする。データ=本文の表と完全一致（相対度数を回数÷合計8000から再計算しassert検算・合計1.00を欄外注記）。軸=横軸22.0〜27.0cm(0.5cmきざみ)・縦軸0〜0.20（0.05刻みの主目盛りのみ・補助目盛り/グリッド線なし——棒の高さから（あ）の値を精密に読み取れないようにするため）。数値ラベルは注記対象の24.0cm(0.18)のみで23.0cmには付けない（練習1の空らん（あ）の解答抑止）。装飾なし・単色。生成方法=assets_provenance/generate_figures.py のパラメトリックSVG（（あ）の解答値を禁止文字列検査で機械排除） -->

...
「Bの当たりやすさはAの結果で変わるのでは？」という違和感は自然だ。実は、**どの時点から見た確率か**で、見えるものが2つに分かれる。

- **引いた後から見ると**——Aが当たりを引いてしまった後なら、Bは絶対に当たらない。この感覚は正しい。
- **引く前から見ると**——樹形図の6通り（当,は1）（当,は2）（は1,当）（は1,は2）（は2,当）（は2,は1）には、Aが当たる未来もはずれる未来も全部ふくまれていて、そのうちBが当たるのは（は1,当）（は2,当）の2通り。だから確率は2/6＝1/3。

いま求めているのは**引く前から見た**確率の方だ。どの時点から見ているかを意識すると、この違和感は解消する（引いた後の話まで数に入れたいなら、それはずっと先の学習になる）。
:::

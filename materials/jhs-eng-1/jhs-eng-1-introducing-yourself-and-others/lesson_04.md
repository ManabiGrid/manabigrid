...
**ここでの説明（生徒向け）**
前の時間に自分の耳が見つけた小さな音の正体は、/s/・/z/・/ɪz/ の3種類。どれになるかは動詞の最後の音で自然に決まる仕組みで、暗記表から生まれた規則ではない。綴りはこの音の記録で、多くは -s、/ɪz/ の仲間は多くが -es（watch → watches）。ただし use のように e で終わる語は、s を足すだけで uses になる。音が先、文字はあと——この順番で付き合うと迷いにくい。（約170字）

**見分けの手順（のどに指を当てて確かめながら・下のフローチャートと同じ流れ）**

1. **まず**、元の動詞の最後の音だけを声に出してみる。シューッと終わる語（watch, teach）なら → /ɪz/。
2. **次に**、のどに指を当てて、もう一度最後の音を出す。指に震えが伝わる語（play, live）なら → /z/。
3. 震えが伝わらない語（like, want）なら → /s/。

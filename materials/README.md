...
**現在28単元＋診断・巻末の2モジュール＋発展モジュール1（計30パッケージ・916ファイル）を同梱しています**（2026-09-02時点。中3数学8単元＋科目診断・巻末資料の2モジュール＋発展モジュール1〔式の展開と因数分解の `advanced/` に同梱・指導要領外のパイロット〕、中2数学6単元、中1数学8単元、数学Ⅰ2単元、中1英語1単元、中学国語1単元、中2理科1単元、中学社会1単元。ファイル数はこのREADME自身を含む実測値）。各単元の工程の到達点（外部レビュー済・QA済・ドラフト等）は [../curriculum/PROGRESS_INDEX.md](../curriculum/PROGRESS_INDEX.md) で確認できます。**人間レビュー（実務者検収＝進捗一覧の「人間レビュー済」への正式検収）が完了するまで、すべて候補ドラフト扱いです。**

最新の実測統計は [../docs/STATS.md](../docs/STATS.md)（自動生成）を参照してください。
...
├── assets/                # 自作SVG図版のみ（借用画像ゼロ・現在393枚）
├── assets_provenance/     # 図版生成スクリプト＋図版台帳（来歴）
└── advanced/              # 発展モジュール（指導要領外の拡張・パイロット。ある単元のみ。標準本文には含まれない）
```

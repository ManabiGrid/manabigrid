...
> 単元総数は **65単元**（中1 20・中2 9・中3 8・数Ⅰ 11・数A〜C 17）＋親ノード13＋モジュール4（うち発展モジュール1）。
...
| `jhs-math-2-linear-function` | 一次関数 | 中2 | 人間レビュー済 | 初期制作単元（Run A）・外部批判レビュー（GPT-5.6 Pro・指摘18件の全件裁定と限定修正）完了 2026-07-19・人間レビュー（現役塾講師による通読）完了 2026-07-19・標準形化（8lesson構成・図版16枚）＋差分レビュー済 2026-08-29 |
...
| `jhs-math-3-expansion-factorization--adv-01` | 中3数学「式の展開と因数分解」発展モジュール adv_01（中高一貫水準・パイロット） | 中3 | 外部レビュー済 | 指導要領外の拡張コンテンツ（親単元 `jhs-math-3-expansion-factorization` の `advanced/` に同梱・標準12レッスンの本文は無変更）。セルフQA＋初見読者検査＋外部批判レビュー（GPT-5.6・OpenAI・全件裁定）＋修正適用 2026-09-02。パイロット版＝需要と水準の検証中で構成変更ありうる |

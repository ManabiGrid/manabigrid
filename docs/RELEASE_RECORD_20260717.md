...
4. 検疫CIの検査パターンは透明性を優先して公開しています（実効的な防御はルールセットのCode Ownersレビュー・required checksです）。検出範囲の設計上の境界: NFKC正規化と不可視文字除去で同定できる表記ゆれまでを機械検査の対象とし、**他の文字体系の同形字（ホモグリフ）や任意の符号化による偽装は機械検査の対象外**です（原理的に列挙不能のため。この層はメンテナの人間レビューで防御します）
